...
import { describe, it, vi, expect as xpt } from "vitest";
...
describe("Button", () => {
  it("should render component with default attributes", async () => {
    const element = await fixture<Button>(
      html`<ds-button>Button</ds-button>`,
    );
  ...
  it("should submit the form when the button type is \"submit\" and event type is \"click\"", () => {
    ...
    element.shouldFormSubmit(new Event("click"));

    xpt(submitSpy).toHaveBeenCalled();
  });

  it("should call handleClick when click in the component", async () => {
    const element = await fixture<Button>(
      html`<ds-button>Button</ds-button>`,
    );
    ...
    const click = vi.spyOn(element, "handleClick");

    element.handleClick(event);

    xpt(click).toHaveBeenCalled();
  });
